...
class CronSchedule
{
     ...
     */
    protected $emailVerificationFactory;
    ...
    public function __construct(
        ...
        $this->emailVerificationFactory = $emailVerificationFactory;
        $this->logger            = $logger;
        $this->helper            = $data;
    }
        ...
        foreach ($collection->getItems() as $cronTab) {
            ...
            switch ($hookType) {
                ...
                case HookType::NEW_CUSTOMER_VERIFICATION:
                    //$item = $this->helper->getBulkOrders($eventID);
                    $item = $this->emailVerificationFactory->create()->load($eventID);
                    break;
            ...
            if($item) $this->helper->send($item, $hookType);
            try {
