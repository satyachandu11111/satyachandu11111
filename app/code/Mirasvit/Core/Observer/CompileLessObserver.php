...
class CompileLessObserver implements ObserverInterface
{
        ...
        if ($this->config->isLessCompilationEnabled()) {
            ...
            if ($this->isPreprocessed()) {
                $pageConfig->addAssets(
                    'Mirasvit_Core::css/source/module.css',
                    [
                        'content_type' => 'css',
                        'src'          => 'Mirasvit_Core::css/source/module.css',
                    ]
                );
            }
        }
    ...
    private function isPreprocessed()
    {
        ...
        $processedData        = file_exists($this->viewSourceBasePath . 'module.less');
    ...
    private function prepareAssetData()
    {
        if (!$this->isPreprocessed()) {
            ...
            if (!empty($filesToImport)) {
                ...
                @file_put_contents($this->viewSourceBasePath . 'module.less', implode(';' . "\n", $filesToImport) . ';');
            }
        }
    }
}
