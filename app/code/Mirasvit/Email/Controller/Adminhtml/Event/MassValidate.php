...
class MassValidate extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if (!is_array($eventIds)) {
                        ...
                        try {
                            ...
                            if (strpos($e->getMessage(), 'Undefined index') !== false) {
                                $this->messageManager->addErrorMessage(
                                    __('Selected event (ID: %1) is not compatible with the trigger "%2".'
                                        . ' Using this trigger you can validate events with the code "%3"',
                                        $eventId,
                                        $trigger->getTitle(),
                                        $trigger->getEvent()
                                    )
            ...
            $this->messageManager->addSuccessMessage(
                __(
                    'Total of %1 event(s) were validated. Passed: %2, failed: %3', count($eventIds), $passed, $failed
                )
            );
        }
