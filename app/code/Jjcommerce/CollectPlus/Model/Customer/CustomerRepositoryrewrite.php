...
class CustomerRepositoryrewrite extends \Magento\Customer\Model\ResourceModel\CustomerRepository
{
    ...
    public function save(\Magento\Customer\Api\Data\CustomerInterface $customer, $passwordHash = null)
    {
        ...
        $objectManager  = $this->customerFactory->getObjectManager();

        $http = $objectManager->get('\Magento\Framework\App\Request\Http');
        $requestString = $http->getRequestString();
        $flag = false;
        if(strpos($requestString, 'V1/carts')) {
            $collectHelper = $objectManager->get('Jjcommerce\CollectPlus\Helper\Data');
            $checkoutSession = $collectHelper->getCheckoutSession();
            $quote = $checkoutSession->getQuote();
            if(is_object($quote) && $quote->getAgentData()) {
                $flag = true;
            }
         }
