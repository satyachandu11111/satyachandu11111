...
class Logger
{
    ...
    {
        $isLogDisabled = $this->scopeConfig->getValue(self::XML_PATH_WELTPIXEL_DEVELOPER_LOGGING, \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $pos = strpos($message, 'Broken reference');
        if ($isLogDisabled && ($pos !== false) ) {
            return false;
        }
