...
namespace Homescapes\Urlrewrite\Observer;
...
class AfterImportDataObserver extends \Magento\CatalogUrlRewrite\Observer\AfterImportDataObserver
{
    ...
    public function __construct(
        ...
        parent::__construct($catalogProductFactory, $objectRegistryFactory, $productUrlPathGenerator, $storeViewService, $storeManager, $urlPersist, $urlRewriteFactory, $urlFinder, $mergeDataProviderFactory, $categoryCollectionFactory);
        $this->connection = $connection;
    }

    public function execute(Observer $observer)
    {
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/import_observer.log');
        ...
        if ($products = $observer->getEvent()->getBunch()) {
            ...
            $temp = $productUrls = $this->generateUrls();
            $logger->info('Test 5');
            if ($flag == 0) {
                foreach($temp as $key => $url)
                {
                    $array = [
                        'entity_type'=>$url->getEntityType(),
                    ...
                    if(!$this->checkIfUrlExists($array)){
                        unset($productUrls[$key]);
                         $logger->info('Test 6');
                    }
                }
            }
    ...
    private function _populateForUrlGeneration($rowData)
    {
        ...
        $logger = new \Zend\Log\Logger();
        $logger->addWriter($writer);
        $logger->info('_populateForUrlGeneration');
    ...
    private function generateUrls()
    {
        ...
        $logger->info('generateUrls 1');

        $mergeDataProvider = clone $this->mergeDataProviderPrototype;
        $logger->info('generateUrls 2');
    ...
    private function currentUrlRewritesRegenerate()
    {
        ...
        $logger->info('currentUrlRewritesRegenerate 1');
        ...
        $logger->info('currentUrlRewritesRegenerate 2');
        ...
        $logger->info('currentUrlRewritesRegenerate 3');    
    ...
    protected function checkIfUrlExists($data)
    {
        $connection= $this->connection->getConnection();
        return (bool)$connection->fetchRow($this->prepareSelect($data));
    }

    protected function prepareSelect($data)
    {
        $connection= $this->connection->getConnection();
        $select = $connection->select('url_rewrite_id');
        $select->from($this->connection->getTableName('url_rewrite'));
