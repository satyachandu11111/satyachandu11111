...
class MassDisapprove extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        foreach ($collection as $auctionProduct) {
            $this->setDisapproveImage($auctionProduct);
        }

        $this->messageManager->addSuccessMessage(
            __(
                'A total of %1 record(s) have not been approved.',
                $collection->getSize()
            ));
     ...
     */
    protected function setDisapproveImage($auctionProduct)
    {
        try {
            $this->model->load($auctionProduct->getId());
            $this->model->setApprove("0");
            $this->model->save();
